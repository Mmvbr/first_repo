...
class NewBall():
    ...
    def __init__(self):
        ...
        self.speed = [randint(-sped, sped), randint(-sped, sped)]
    ...
    def in_field(self):
        ...
        if not condition1 or not condition2:
            self.speed[0] = -self.speed[0] * randint(1, 30) / 10 if abs(self.speed[0]) < 100 else 10
        if not condition3 or not condition4:
            self.speed[1] = -self.speed[1] * randint(1, 30) / 10 if abs(self.speed[1]) < 100 else 10
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            for i in range(n_balls):
                if balls[i].check(mouse_x, mouse_y):
                     balls[i] = NewBall()
                     cnt+=1
